...
%Account
\node (acc) [class] {Cuenta \nodepart{second} balance \nodepart{third} abrir \\ cerrar \\ orden};
	
%Customer
\node (cust) [class,xshift=10cm, yshift=4cm] {\textit{Cliente}\nodepart{second}id};
\node (accUse) [class,below of =cust,xshift=-2cm, yshift=-2cm] {UsuarioCuenta\nodepart{second} \nodepart{third}};
\node (ownRol) [class,below of=cust,xshift=2cm, yshift=-2cm] {Due\~no\nodepart{second} infoCr\'edito \nodepart{third}abrirCuenta()\\cerrarCuenta()};
 \draw [inherentFlow] (accUse.north) -- +(0pt,20pt)-|(cust);
 \draw [inherentFlow] (ownRol.north) -- +(0pt,10pt)-|(cust);


%Transaction
\node (trans) [class,xshift=4cm, yshift=5.5cm] {Transacci\'on \nodepart{second}\nodepart{third} dep\'osito \\ retiro \\ orden};
	
%Employee
\node (empl) [class,xshift=0cm, yshift=17cm,align=left] {\textit{Empleado}\nodepart{second}nombre \\ id \\ salario};
\node (broker) [class,below of=empl,xshift=2cm, yshift=-2cm] {Agente\nodepart{second} \nodepart{third}modificarOrden() \\ cerrarOrder()\\actualizarCuenta()};
\node (manag) [class,below of=empl,xshift=-2cm, yshift=-2cm] {Administrador\nodepart{second} \nodepart{third}abrirCuenta()\\cerrarCuenta()};

\draw [inherentFlow] (broker.north) -- +(0pt,14pt)-| (empl);
\draw [inherentFlow] (manag.north) -- +(0pt,19pt)-| (empl);
	 
%Order
\node (order) [class,above of =cust,yshift=7cm,xshift=1cm] {Orden\nodepart{second} \nodepart{third}abrir\\cerrar};


 
%Auditor
\node (audi) [class,right of =order,xshift=4cm] {Auditor\nodepart{second}nombre \\ id \nodepart{third}verificarInfoOren()};

%Report
\node (report) [class,right of =audi,yshift=-7.5cm,xshift=1cm] {Informe\nodepart{second} \nodepart{third}crear\\modificar};

%patron de regulación para auditoría 
\draw [](7,13.6) node[patNote] (patR1) {Auditor\'ia en intervalos \\  espec\'ificos} (patR1.north east) ;
\filldraw [fill=coolgrey,draw=black](patR1.north east)--++(-3.8pt,3.9pt)--++(0,-8pt)--++(7.9pt,0)--(patR1.north east);%lecturaopt5
	
%Rights from RBAC pattern 
\node (ownRig) [derecho,xshift=7cm, yshift=-6cm] {DerechoDue\~no\nodepart{second}tipoAcceso};
\draw[] (10,-6.3) node[draw=black,note] (opt1) {abrir, \\ cerrar} ;
\filldraw [fill=coolgrey,draw=black] (opt1.north east)--++(-3.8pt,3.9pt)--++(0,-8pt)--++(7.9pt,0)--(opt1.north east);%opt1

\node (accRig) [derecho,xshift=4cm, yshift=-2cm] {DerechoUsuario\nodepart{second}tipoAcceso};
...
\node (audRig) [derecho,xshift=13.8cm, yshift=15.5cm] {DerechoAuditor\nodepart{second}tipoAcceso};
\draw [](17,15.2) node[draw=black,note] (opt3) {lectura} (opt3.north east); node[]() {}
\filldraw [fill=coolgrey,draw=black] (opt3.north east)--++(-3.8pt,3.9pt)--++(0,-8pt)--++(7.9pt,0)--(opt3.north east);%opt3

\node (broRig) [derecho,xshift=4cm, yshift=8.4cm] {DerechoAgente\nodepart{second}tipoAcceso};
\draw [](7,8.1) node[draw=black,note] (opt4) {lectura\\trade} (opt4.north east);
\filldraw [fill=coolgrey,draw=black] (opt4.north east)--++(-3.8pt,3.9pt)--++(0,-8pt)--++(7.9pt,0)--(opt4.north east);%opt4

\node (manRig) [derecho,text width=3.4cm,xshift=-3cm, yshift=6.95cm] {DerechoAdministrador\nodepart{second}tipoAcceso};
\draw [](-2,4) node[note] (opt5) {abrir\\cerrar\\administrar} (opt5.north east);
\filldraw [fill=coolgrey,draw=black] (opt5.north east)--++(-3.8pt,3.9pt)--++(0,-8pt)--++(7.9pt,0)--(opt5.north east);%opt5

\node (audRig1) [derecho,xshift=23cm, yshift=9cm] {DerechoAuditor\nodepart{second}tipoAcceso};
\draw [](24,7) node[draw=black,note] (opt6) {crear\\lectura\\modificar} (opt6.north east); node[]() {}
\filldraw [fill=coolgrey,draw=black] (opt6.north east)--++(-3.8pt,3.9pt)--++(0,-8pt)--++(7.9pt,0)--(opt6.north east);%opt6

%Patterns

\draw [ATX](13.5,5.5) node [patron] (authTx) {\textit{TX} \\ \textit{authenticator}} (authTx.north west);
\filldraw [fill=azul,draw=black] (authTx.north west) -- ++(0,0.3)-- ++(1.2,0)--++(0,-0.3)--(authTx.north west);

\draw[SLAA] (11,9) node [patron] (secLog) {\textit{Security logger} \\ \textit{and auditor}} (secLog.north west);
\filldraw [fill=azul,draw=black] (secLog.north west)  -- ++(0,0.3)-- ++(1.2,0)--++(0,-0.3)--(secLog.north west);

\draw[SLAA] (-0.7,-6) node [patron] (secLog2) {\textit{Security logger} \\ \textit{and auditor}} (secLog2.north west);
\filldraw [fill=azul,draw=black] (secLog2.north west) -- ++(0,0.3)-- ++(1.2,0)--++(0,-0.3)--(secLog2.north west);

\draw[SLAA] (20,0) node [patron] (secLog3) {\textit{Security logger} \\ \textit{and auditor}} (secLog3.north west);
\filldraw [fill=azul,draw=black] (secLog3.north west) -- ++(0,0.3)-- ++(1.2,0)--++(0,-0.3)--(secLog3.north west);

\draw[] (17,9) node [patron] (authR) {\textit{Authenticator}} (authR.north west);
